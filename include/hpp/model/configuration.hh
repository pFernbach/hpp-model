...
namespace hpp {
  namespace model {
    ...
    void integrate  (const DevicePtr_t& robot,
		     ...
		     const vector_t& velocity, Configuration_t& result)
    {
    ...
    /// $\f\textbf{v}\f$
    void difference (const DevicePtr_t& robot, const Configuration_t& q1,
		     const Configuration_t& q2, vector_t& result)
    {
      const JointVector_t& jv (robot->getJointVector ());
      for (model::JointVector_t::const_iterator itJoint = jv.begin ();
	   itJoint != jv.end (); itJoint++) {
